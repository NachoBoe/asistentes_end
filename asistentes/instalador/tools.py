...
index_name = "api-index-clean"
credential = AzureKeyCredential(os.getenv("AZURE_AI_SEARCH_API_KEY"))
endpoint = os.getenv("AZURE_AI_SEARCH_SERVICE_NAME")
embeddings = AzureOpenAIEmbeddings(model="text-embedding-3-large", azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"), api_key=os.getenv("AZURE_OPENAI_API_KEY"))
...
class method_decription(BaseModel):
    ...
    sistem: str = Field(description="Sistem to which filter the search", default="all")
...
def get_methods_from_description(method_description:str, sistem: str="all"):
    """ Returns a list of possible methods provided a description. Returns the name, sistem, brief description and inputs/outputs of the methods. """
    ...
    vector_query = VectorizedQuery(vector=embeddings.embed_query(method_description), k_nearest_neighbors=50, fields="DescriptionVector", exhaustive=True)
    results = search_client.search(  
        search_text=search_str,  
        vector_queries=[vector_query],
        select=["nombre", "sistema", "descripcion","entrada","salida"],
        query_type=QueryType.SEMANTIC, semantic_configuration_name='my-semantic-config', query_caption=QueryCaptionType.EXTRACTIVE, query_answer=QueryAnswerType.EXTRACTIVE,
        top=10,
    )
    ret_metods = []
    for res in results:
        ret_metods.append((res["nombre"],res["sistema"],res["descripcion"],res["entrada"],res["salida"]))
...
def get_method_info(method:str, sistem: str):
    """ Returns information (sistem, description, input/output) of a method."""
    ...
    ret_metod = []
    for res in results:
        ret_metod.append((res["nombre"],res["sistema"],res["descripcion"], res["entrada"], res["salida"]))
    ...
    resp =  method_info_as_string(ret_metod)
    return resp

## TOOL 2
@tool("get_method_info", args_schema=method_name)
def get_method_for_ids(method:str, sistem: str):
    """ Returns information of the method used to get the Ids of a given entity."""
...
tools = [get_methods_from_description, get_method_info]#,get_all_method_from_sistem]
